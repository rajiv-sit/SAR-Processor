...
TEST(PipelineRunnerTests, RtoPreviewFailsWithInvalidEndpoint) {
    ...
    EXPECT_FALSE(runner.runRtoPreview("udp://", std::nullopt, std::nullopt, 8, 8, 1, 0));
}

TEST(PipelineRunnerTests, RtoPreviewPublishesFrameWithDefaults) {
    app::PipelineRunner runner;
    EXPECT_TRUE(runner.runRtoPreview("udp://127.0.0.1:5005", std::nullopt, std::nullopt, 0, 0, 0, 0));
}
